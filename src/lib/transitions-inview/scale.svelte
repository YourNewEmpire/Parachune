<script lang="ts">
  ...
  import { type ScaleParams, scale } from "svelte/transition";
  import viewport from "$lib/utils/useViewportAction";

  export let options: ScaleParams = {
    delay: 0,
    duration: 400,
    easing: cubicOut,
    start: 0,
  ...
  export let outOptions: ScaleParams | "static" | "off" = "off";
...
{:else if outOptions !== "off"}
  ...
  >
    {#if visible}
      <div in:scale={options} out:scale={outOptions}>
        <slot />
      </div>
    {/if}
  </div>
